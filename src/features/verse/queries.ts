...
import { type SearchResult, type VerseRef, verseRepository } from './repository';
...
export const searchVerses = cache(async (query: string, limit?: number) => {
  return verseRepository.search(query, limit);
});
